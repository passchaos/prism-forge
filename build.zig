...
pub fn build(b: *std.Build) void {
    ...
    const mod = b.addModule("prism_forge", .{
    ...
    const mnist_exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            .imports = &.{.{ .name = "prism_forge", .module = mod }},
            .target = target,
        }),
    });
    const mnist_cmd = b.addRunArtifact(mnist_exe);
    // mnist_cmd.step.dependOn(mnis)
    ...
    const exe_check = b.addExecutable(.{ .name = "exe_check", .root_module = mod });
